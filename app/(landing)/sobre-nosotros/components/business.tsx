...
import { AboutUs, StrapiAboutUsEntry } from '../interfaces/aboutUs.interface'
...
export const Business = ({ data }: { data: StrapiAboutUsEntry }) => {
  return (
    <div className="w-full bg-[#0b1d3a] rounded-2xl px-12 py-14 my-16 flex flex-col items-center">
      <h2 className="text-2xl font-bold text-white mb-1">{data.businessCard?.titulo}</h2>
      <h3 className="text-xl text-slate-300 font-light mb-10">
        {data.businessCard?.subtitulo}
      </h3>
      <p className="text-sm text-slate-400 text-center max-w-2xl mb-14 leading-relaxed">
        {data.businessCard?.descripcion}
      </p>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 w-full">
        {data.businessCard?.caracteristicas?.map((feature, index) => (
