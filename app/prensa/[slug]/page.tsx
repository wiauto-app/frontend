...
import { PressCommentForm } from "../components/PressCommentForm";
import { PressCommentsList } from "../components/PressCommentsList";
import { PressBlocksContent } from "../components/PressBlocksContent";
...
export const generateMetadata = async ({
  ...
  try {
    const press = await pressService.findOne({ slug });
    return {
      title: press.seo?.metaTitle ?? press.title,
      description: press.seo?.metaDescription ?? press.summary,
    };
  } catch {
    return { title: "Prensa" };
  }
...
const ShareWhatsApp = () => (
  <a href="#" aria-label="Compartir en WhatsApp"
...
const ShareLink = () => (
  <a href="#" aria-label="Copiar enlace"
...
export default async function PressDetailPage({ params }: PressDetailPageProps) {
  ...
  let press: Awaited<ReturnType<typeof pressService.findOne>>;

  try {
    press = await pressService.findOne({ slug });
  } catch {
    notFound();
  }

  const primary_banner = press.banners[0] ?? null;
  ...
  const displayComments = press.comments.length > 0 ? press.comments : mockComments;
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 -mt-44">
        <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl bg-slate-100 shadow-lg mb-8">
          {primary_banner ? (
            <Image src={primary_banner.url}
              alt={primary_banner.alternative_text ?? press.title} fill unoptimized
              className="object-cover" priority
              sizes="(max-width: 768px) 100vw, 896px" />
          ) : (
            <Image src={MOCK_BANNER}
              alt={press.title} fill unoptimized
              className="object-cover" priority
              sizes="(max-width: 768px) 100vw, 896px" />
          )}
        </div>
      ...
      <article className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 pb-16">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 mb-6 border-b border-slate-100 pb-5">
          {press.category && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-medium text-blue-600">{press.category.name}</span>
            </div>
          )}
          {press.publisher && (
            <div className="flex items-center gap-1.5">
              <User className="h-4 w-4 text-slate-400" />
              <span>{press.publisher.name}</span>
            </div>
          )}
          {press.published_at && (
            <div className="flex items-center gap-1.5">
              <Calendar className="h-4 w-4 text-slate-400" />
              <time dateTime={press.published_at}>{formatDate(press.published_at)}</time>
            </div>
          )}
          <div className="flex items-center gap-1.5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-full bg-slate-200 overflow-hidden flex-shrink-0 relative">
              {press.publisher?.image_url ? (
                <Image src={press.publisher.image_url} alt={press.publisher.name} fill className="object-cover" unoptimized />
              ) : (
                <div className="flex h-full w-full items-center justify-center text-slate-400 text-xs font-bold bg-slate-200">
                  {(press.publisher?.name ?? "A").charAt(0).toUpperCase()}
                </div>
              )}
            </div>
            <span className="text-sm font-semibold text-slate-800">
              {press.publisher?.name ?? "Cameron Williamson"}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <ShareWhatsApp /><ShareFacebook /><ShareTwitter /><ShareLinkedIn /><SharePinterest /><ShareLink />
          </div>
        </div>

        {press.summary && (
          <p className="text-base leading-relaxed text-slate-700 mb-6">{press.summary}</p>
        )}
        ...
        {press.citation && (
          <blockquote className="my-8 rounded-r-xl border-l-4 border-blue-600 bg-[#E8F0FE] py-5 pl-8 pr-6">
            ...
            <p className="text-base font-medium leading-relaxed text-slate-800">{press.citation}</p>
          </blockquote>
        )}

        <div className="mt-2">
          <PressBlocksContent content={press.content} />
        </div>

        <section className="mt-14 border-t border-slate-100 pt-10">
          <PressCommentForm news_document_id={press.document_id} news_slug={press.slug} />
          <div className="mt-10">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Comentarios</h2>
            <PressCommentsList comments={displayComments} />
          </div>
        </section>
      </article>
    </div>
